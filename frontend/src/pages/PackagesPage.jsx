...
const DEFAULT_SUPPORT_PHONE = '972500000000';

// Normalize phone for wa.me — strip non-digits, ensure country code
function normalizePhone(raw) {
  if (!raw) return DEFAULT_SUPPORT_PHONE;
  let p = String(raw).replace(/\D/g, '');
  if (!p) return DEFAULT_SUPPORT_PHONE;
  // 0XXXXXXXXX → 972XXXXXXXXX
  if (p.startsWith('0')) p = '972' + p.slice(1);
  // bare 5XXXXXXXX (Israeli mobile without leading zero)
  else if (p.length === 9 && p.startsWith('5')) p = '972' + p;
  return p;
}

function buildWhatsAppLink(phone, planLabel) {
  const msg = `היי אלעד, אני מעוניין להצטרף לחבילת ${planLabel}`;
  return `https://wa.me/${normalizePhone(phone)}?text=${encodeURIComponent(msg)}`;
}

export default function PackagesPage({ settings }) {
  const supportPhone = settings?.support_phone || DEFAULT_SUPPORT_PHONE;
  ...
  return (
    <div className="flex-1 px-4 md:px-6 py-6">
      ...
      <div className="grid md:grid-cols-3 gap-5 max-w-5xl mx-auto">
        {PLANS.map(plan => {
          const isCurrent = plan.key === 'pro';
          const waHref = buildWhatsAppLink(supportPhone, plan.label);
            ...
            <div key={plan.key} className="rounded-2xl p-6 flex flex-col relative"
              ...
              {isCurrent ? (
                <button disabled
                  ...
                  style={{ background: `${plan.color}20`, color: plan.color, border: `1px solid ${plan.color}40`, opacity: 0.85 }}>
                  ✓ החבילה הנוכחית
                </button>
              ) : (
      ...
      <div className="mt-10 card rounded-2xl p-6 max-w-2xl mx-auto text-center">
        ...
        <a href={`https://wa.me/${normalizePhone(supportPhone)}?text=${encodeURIComponent('שלום, אני מעוניין בחבילה מותאמת לסוכנות שלי')}`}
